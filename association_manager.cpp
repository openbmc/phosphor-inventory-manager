...
bool Manager::loadConditions()
{
    ...
    for (const auto& dirent : fs::recursive_directory_iterator(dir))
    {
        ...
        {
            if (path.string().find(".json") != std::string::npos)
            {
                ...
                if (json.is_object() && json.contains("condition"))
                {
                    Condition c;
                    c.file = path;
                    c.path = json["condition"]["path"].get<std::string>();
                    if (c.path.front() != '/')
                    {
                        c.path = '/' + c.path;
                    }
                    ...
                    c.interface =
                        json["condition"]["interface"].get<std::string>();
                    c.property =
                        json["condition"]["property"].get<std::string>();

                    // The values are in an array, and need to be
                    // converted to an InterfaceVariantType.
                    for (const auto& value : json["condition"]["values"])
                    {
                        if (value.is_array())
                        {
                        ...
                        {
                            std::stringstream ss;
                            ss << "Invalid condition property value in "
                               << c.file << ": " << value;
