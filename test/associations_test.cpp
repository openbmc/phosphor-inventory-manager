...
class AssocsTest : public ::testing::Test
{
  protected:
    ...
    std::string writeFile(const char* data, const std::string& fileName)
    {
        fs::path path = jsonDir / fileName;
...
{
    auto path = writeFile(goodJson, "good.json");
    Manager m(bus, path);
...
{
    auto path = writeFile(badJson0, "bad0.json");
    Manager m(bus, path);
...
{
    auto path = writeFile(badJson1, "bad1.json");
    Manager m(bus, path);
...
{
    auto path = writeFile(badJson2, "bad2.json");
    Manager m(bus, path);
...
{
    auto path = writeFile(badJson3, "bad3.json");
    Manager m(bus, path);
