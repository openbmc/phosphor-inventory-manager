...
std::tuple<std::string, std::string> writeFile(const char* data)
{
    char dir[] = {"assocTestXXXXXX"};
    fs::path jsonDir = mkdtemp(dir);
    ...
    return std::tuple<std::string, std::string>(jsonDir, path);
}

TEST(ASSOCS_TEST, TEST_NO_JSON)
{
    auto bus = sdbusplus::bus::new_default();
    Manager m{bus};
    EXPECT_EQ(m.getAssociationsConfig().size(), 0);
}

TEST(ASSOCS_TEST, TEST_GOOD_JSON)
{
    auto bus = sdbusplus::bus::new_default();
    auto [dir, path] = writeFile(goodJson);
    Manager m(bus, path);
...
TEST(ASSOCS_TEST, TEST_BAD_JSON0)
{
    auto bus = sdbusplus::bus::new_default();
    auto [dir, path] = writeFile(badJson0);
    Manager m(bus, path);
...
TEST(ASSOCS_TEST, TEST_BAD_JSON1)
{
    auto bus = sdbusplus::bus::new_default();
    auto [dir, path] = writeFile(badJson1);
    Manager m(bus, path);
...
TEST(ASSOCS_TEST, TEST_BAD_JSON2)
{
    auto bus = sdbusplus::bus::new_default();
    auto [dir, path] = writeFile(badJson2);
    Manager m(bus, path);
...
TEST(ASSOCS_TEST, TEST_BAD_JSON3)
{
    auto bus = sdbusplus::bus::new_default();
    auto [dir, path] = writeFile(badJson3);
    Manager m(bus, path);
